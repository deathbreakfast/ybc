...
impl Component for Pagination {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let mut pagination_classes = Classes::from("pagination");
        pagination_classes.push(classes);
        if let Some(size) = &size {
            pagination_classes.push(&size.to_string());
        }
        if let Some(alignment) = &alignment {
            pagination_classes.push(&alignment.to_string());
        }
        if *rounded {
            pagination_classes.push("is-rounded");
        }
        html! {
            <nav class={pagination_classes} role="navigation" aria-label="pagination">
                {previous.clone()}
                {next.clone()}
...
impl Component for PaginationItem {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let PaginationItemProps { children, label, onclick, item_type } = ctx.props();
        html! {
            <a class={item_type.to_string()} aria-label={label.clone()} onclick={onclick.clone()}>
                {children.clone()}
            </a>
        }
    }
